...
void TCPReceiver::segment_received(const TCPSegment &seg) {
    ...
    if (head.syn == true)
        this->_reassembler.push_substring(
            seg.payload().copy(), unwrap(head.seqno, this->_isn, this->_reassembler.next_expected_index()), head.fin);
    else
        this->_reassembler.push_substring(seg.payload().copy(),
                                          unwrap(head.seqno, this->_isn, this->_reassembler.next_expected_index()) - 1,
                                          head.fin);
}
