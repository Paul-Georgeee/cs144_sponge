...
//! has a different ISN.
static inline uint64_t getabs(int64_t a) { return a < 0 ? -a : a; }
...
uint64_t unwrap(WrappingInt32 n, WrappingInt32 isn, uint64_t checkpoint) {
    ...
    // uint64_t differ_a = abs(int64_t(tmp_a) - int64_t(checkpoint)), differ_b = abs(int64_t(tmp_b) -
    // int64_t(checkpoint)), differ_c = abs(int64_t(tmp_c) - int64_t(checkpoint));

    uint64_t differ_a = max(tmp_a, checkpoint) - min(tmp_a, checkpoint),
             differ_b = max(tmp_b, checkpoint) - min(tmp_b, checkpoint),
             differ_c = max(tmp_c, checkpoint) - min(tmp_c, checkpoint);
